...
export default memo(
  ({
    ...
    childRoot?: EntityNode;
    node: EntityNode;
    parentRoot?: EntityNode;
  }) => {
    ...
    useEffect(() => {
      if (node.data.geniId) {
        getGeniProfile(node.data.geniId).then((geniProfile) => {
    ...
    useEffect(() => {
      if (settings.secondLabelCode) {
        if (isProperyId(settings.secondLabelCode)) {
          //TODO get not from simpleclaim
          const secondLabel = getSimpleClaimValue(
          ...
          <>
            {node.data.leftIds && !!node.data.leftIds.length && (
              <Button
                ...
                title={
                  (node.data.siblings ? "Collapse" : "Expand") + " siblings"
                }
              >
            ...
            {!!node.data.upIds?.length && (
              <Button
                ...
                variant="link"
                disabled={(parentRoot || node).data.loadingParents}
                onClick={() => dispatch(toggleParents(parentRoot || node))}
              >
                <span className="value">{node.data.upIds.length}</span>
                <span className="chevron ml-1 mr-1">
                  {(parentRoot || node).data.parents ? (
                    <FiChevronDown />
                  ) : (
                    <FiChevronUp />
                  )}
            ...
            {!!node.data.downIds?.length && (
              <Button
                ...
                variant="link"
                disabled={(childRoot || node).data.loadingChildren}
                onClick={() => dispatch(toggleChildren(childRoot || node))}
              >
                <span className="value">{node.data.childrenCount}</span>
                <span className="chevron ml-1 mr-1">
                  {(childRoot || node).data.children ? (
                    <FiChevronUp />
                  ) : (
                    <FiChevronDown />
                  )}
